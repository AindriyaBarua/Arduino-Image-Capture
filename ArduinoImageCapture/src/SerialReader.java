...
/*Roughly speaking SerialReader is just a wrapper for the rxtx library.
It just starts the reading process from com port and forwards all the received bytes to
ImageCapture.
The function names here should be pretty self-explanatory 

Basically when “Listen” button is clicked then 
“serialReader.startListening(String portName, Integer baudRate)” is called
This calls “openPort” that opens com port for reading and starts waiting for bytes.

If it receives a byte it calls “addReceivedByte(byte receivedByte)” of the ImageCapture object.*/
